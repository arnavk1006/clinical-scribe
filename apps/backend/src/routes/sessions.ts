...
const routes = new Hono()
  ...
  .post(
    ...
    validator("json", (value, c) => {
      const body = value as {
        patientId?: string;
        doctorId?: string;
        status?: "recording" | "processing" | "transcribed" | "reviewed";
        audioPath?: string;
      };
    ...
    async (c) => {
      try {
        const { patientId, doctorId, status, audioPath } = c.req.valid("json");
  ...
  .patch(
    ...
    async (c) => {
      const id = c.req.param("id");
      try {
        const { status, audioPath, endedAt } = c.req.valid("json");
        ...
        if (status !== undefined) {
          ...
          updates.status = status as "recording" | "processing" | "transcribed" | "reviewed";
        }
